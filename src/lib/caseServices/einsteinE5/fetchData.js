...
var _einsteinSdk = _interopRequireDefault(require("einstein-sdk"));
...
class FetchData {
  constructor({
    ...
    this.einsteinSDK = new _einsteinSdk.default(servicesUrl);
    this.itemId = null;
  ...
  async getCaseByExternalId(externalId) {
    try {
      ...
      const {
        item
      } = await this.einsteinSDK.getTestCase(this.itemId);
      ...
      if (item.children) {
        item.children.forEach(element => {
          ...
          element.expectedResult = expectedResult && this._format(expectedResult);
